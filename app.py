...
with st.sidebar:
    ...
    is_mobile = False
    st.markdown("---")
    st.write("Lunga Ndzimandze © 2026")
...
if selected == "Home":
    ...
    # Dashboard Metrics
    # Compute dynamic risk areas
    fire_df_home = df[df['label']==1]
    risk_areas_count = fire_df_home.groupby(['latitude', 'longitude']).ngroups
    
    if is_mobile:
        st.metric("Total Records", f"{meta['data_stats']['total_observations']:,}")
        st.metric("Historical Fires", f"{meta['data_stats']['fire_events']:,}")
        st.metric("Risk Areas", f"{risk_areas_count:,}")
        st.metric("Model AUC", f"{meta['metrics']['auc']:.4f}")
        st.metric("Model F1", f"{meta['metrics']['f1_score']:.4f}")
    else:
...
# Data Analysis
    ...
    with tabs[3]:
        ...
        q1.metric("Missing Values", f"{missing}")
    ...
    with col_res:
        if submit:
            with st.spinner("Processing Model..."):
                ...
                Xnew = pd.DataFrame([features])[meta['features']]
        ...
        if 'pred_features' in st.session_state:
            ...
            X_sc = pd.DataFrame([features_sc])[meta['features']]
            ...
            st.markdown(f"**Scenario Risk:** `{prob_sc:.2%}` (<span style='color:{diff_color}'>{arrow} {abs(diff*100):.1f}% point shift</span>)", unsafe_allow_html=True)
